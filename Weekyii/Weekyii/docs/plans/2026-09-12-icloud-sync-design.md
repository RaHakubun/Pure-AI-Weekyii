...
设备本地保留：通知授权与排程、默认 kill time、执行模式、主题与界面偏好、默认项目时长、开发者设置、演示数据设置。它们在另一台设备上可以不同，这是首版明确的产品边界，不属于同步失败。`TaskTypeDefinition`（含用户创建的任务类型）属于业务数据，会同步；`UserSettings.defaultTaskTypeIdRaw` 只是每台设备自己的默认选择。`AppState` 中可由业务数据推导的计数和刷新 revision 不作为云端事实来源。
...
同样的修复会在持久化容器启动阶段先执行，再进行一致性校验。这可以处理应用上次退出前已落入本地缓存的远端冲突，避免重复“当前周”等可恢复状态导致下次启动失败。

所有业务入口在插入周或日期前，先按 `weekId` / `dayId` 幂等查询并复用已有对象。当两台离线设备仍然同时创建同一 `weekId` / `dayId` 时，修复器会合并重复周与日期，将双方任务迁入同一日期后再规范 Focus 和顺序。内置任务类型也按固定 `idRaw` 收敛。随机 UUID 标识的普通任务不做语义去重，以免误删用户分别创建但文本相同的任务。

## 失败策略

- 未登录 iCloud、账号受限或网络不可用：继续使用本地副本，设置页显示明确状态。
- 本地数据库迁移失败：沿用 fail-closed 行为，停止写入，并允许重试或从最近有效恢复点还原。
- CloudKit 上传失败：不删除本地数据，等待系统重试。
- 首次升级：每个 Schema 版本在迁移前建立一次本地 SQLite 快照；快照包含外部附件目录、使用流式校验、最多保留 8 份，并排除出系统设备备份。数据导入仍单独建立恢复点。
